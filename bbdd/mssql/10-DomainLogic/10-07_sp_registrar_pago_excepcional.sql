CREATE OR ALTER PROCEDURE core.sp_Registrar__Pago_Excepcional(@json NVARCHAR(MAX)) AS
	BEGIN
		SET NOCOUNT ON;
		SET XACT_ABORT ON;
		...
		BEGIN TRY
			--parse de json
				...
				INSERT INTO #Detalle_Pago(nro_socio, periodo, monto, comentario)
				SELECT 
					j2.nro_socio,
					j2.periodo,
					j2.monto,
			...
			--validamos que pago y detalle de pago tenga algo
				IF NOT EXISTS(SELECT 1 FROM #Pago) 
					OR 
				   NOT EXISTS(SELECT 1 FROM #Detalle_Pago)
			...
			--validamos si la SIGUIENTE cuota no paga, coincide con la que se tiene que pagar
				DECLARE @primerPeriodo TABLE (nro_socio INT PRIMARY KEY, primer_periodo DATE);
			   	INSERT INTO @primerPeriodo(nro_socio, primer_periodo)
			    SELECT
			        nro_socio,
			        MIN(periodo)
			    FROM #Detalle_Pago
			...
			BEGIN TRANSACTION
				...
				-- inserto a tabla Pagos
					...
					SELECT 
						...
						medio_pago,
						origen_carga,
						(SELECT SUM(monto) FROM #Detalle_Pago),
						comentario
					FROM #Pago;
				...
				-- inserto a tabla Detalles_del_Pago
					...
					SELECT
						...
						dp.nro_socio, --para esto tuve que corrobar que existian esos socios (esta hecho antes)
						dp.periodo,
						dp.monto,
						dp.comentario
					FROM #Detalle_Pago dp;
				...
				-- Registrar evento de pago registrado
					...
					SELECT 
						...
						'Periodo ' + CAST(periodo AS VARCHAR(10)),
						'Pago', CAST(@id_pago_generado AS VARCHAR(10)),
						'Monto de $' + CAST(monto AS VARCHAR(20))
					FROM #Detalle_Pago;
			COMMIT;
			EXEC internal.sp_Registrar__Ejecucion_Proceso @sp_name;
			EXEC sp_set_session_context @key = N'Origen', @value = NULL;

    		PRINT '[INFO] Se registro el pago excepcional en el sistema';
		END TRY
		BEGIN CATCH
	...
	END;
GO
PRINT '[INFO] sp_Registrar__Pago_Excepcional Creada';
GO
